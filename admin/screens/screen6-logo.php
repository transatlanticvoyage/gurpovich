...
class Screen6_Logo {
    public function render() {
        echo '<div class="wrap">';
        echo '<div style="font-weight:bold; font-size:1.2em; margin-bottom:10px;">Screen 6 - Logo</div>';
        echo '<h1>Logo Content Injector</h1><form method="post">';
        wp_nonce_field('gurpovich_logo_action','gurpovich_logo_nonce');
        ...
        if (isset($_POST['gurp_inject'])) {
            if (!isset($_POST['gurpovich_logo_nonce']) || !wp_verify_nonce($_POST['gurpovich_logo_nonce'],'gurpovich_logo_action')) {
            ...
            // Save mapping meta
            update_post_meta($post_id,'gurp_logo_map',$map);
            echo '<div class="updated"><p>Logo mapping saved.</p></div>';
        ...
        // Show mapping
        echo '<h2>Current Logo Mapping</h2>';
        if (!empty($_POST['gurp_post_id'])) {
            $existing = get_post_meta(intval($_POST['gurp_post_id']),'gurp_logo_map',true);
            if (is_array($existing)) {
                echo '<pre>' . esc_html(print_r($existing,true)) . '</pre>';
            } else {
                echo '<p>No logo mapping.</p>';
            }
        }
