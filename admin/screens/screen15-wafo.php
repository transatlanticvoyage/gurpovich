...
class Screen15_Wafo {
    public function render() {
        echo '<div class="wrap">';
        echo '<div style="font-weight:bold; font-size:1.2em; margin-bottom:10px;">Screen 15 - wafo dummy seo content</div>';
        echo '<h1>Wafo Dummy SEO Content Injector</h1><form method="post">';
        wp_nonce_field('gurpovich_wafo_action','gurpovich_wafo_nonce');
        ...
        if (isset($_POST['gurp_inject'])) {
            if (!isset($_POST['gurpovich_wafo_nonce']) || !wp_verify_nonce($_POST['gurpovich_wafo_nonce'],'gurpovich_wafo_action')) {
            ...
            // Save mapping meta
            update_post_meta($post_id,'gurp_wafo_map',$map);
            echo '<div class="updated"><p>Wafo dummy SEO content mapping saved.</p></div>';
        ...
        // Show mapping
        echo '<h2>Current Wafo Dummy SEO Content Mapping</h2>';
        if (!empty($_POST['gurp_post_id'])) {
            $existing = get_post_meta(intval($_POST['gurp_post_id']),'gurp_wafo_map',true);
            if (is_array($existing)) {
                echo '<pre>' . esc_html(print_r($existing,true)) . '</pre>';
            } else {
                echo '<p>No wafo dummy SEO content mapping.</p>';
            }
        }
