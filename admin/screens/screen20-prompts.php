...
class Screen20_Prompts {
    public function render() {
        echo '<div class="wrap">';
        echo '<div style="font-weight:bold; font-size:1.2em; margin-bottom:10px;">Screen 20 - prompts</div>';
        echo '<h1>Prompts Manager</h1><form method="post">';
        wp_nonce_field('gurpovich_prompts_action','gurpovich_prompts_nonce');
        ...
        if (isset($_POST['gurp_inject'])) {
            if (!isset($_POST['gurpovich_prompts_nonce']) || !wp_verify_nonce($_POST['gurpovich_prompts_nonce'],'gurpovich_prompts_action')) {
            ...
            // Save mapping meta
            update_post_meta($post_id,'gurp_prompts_map',$map);
            echo '<div class="updated"><p>Prompts mapping saved.</p></div>';
        ...
        // Show mapping
        echo '<h2>Current Prompts Mapping</h2>';
        if (!empty($_POST['gurp_post_id'])) {
            $existing = get_post_meta(intval($_POST['gurp_post_id']),'gurp_prompts_map',true);
            if (is_array($existing)) {
                echo '<pre>' . esc_html(print_r($existing,true)) . '</pre>';
            } else {
                echo '<p>No prompts mapping.</p>';
            }
        }
