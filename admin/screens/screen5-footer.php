...
class Screen5_Footer {
    public function render() {
        echo '<div class="wrap">';
        echo '<div style="font-weight:bold; font-size:1.2em; margin-bottom:10px;">Screen 5 - Footer</div>';
        echo '<h1>Footer Content Injector</h1><form method="post">';
        wp_nonce_field('gurpovich_footer_action','gurpovich_footer_nonce');
        ...
        if (isset($_POST['gurp_inject'])) {
            if (!isset($_POST['gurpovich_footer_nonce']) || !wp_verify_nonce($_POST['gurpovich_footer_nonce'],'gurpovich_footer_action')) {
            ...
            // Save mapping meta
            update_post_meta($post_id,'gurp_footer_map',$map);
            echo '<div class="updated"><p>Footer mapping saved.</p></div>';
        ...
        // Show mapping
        echo '<h2>Current Footer Mapping</h2>';
        if (!empty($_POST['gurp_post_id'])) {
            $existing = get_post_meta(intval($_POST['gurp_post_id']),'gurp_footer_map',true);
            if (is_array($existing)) {
                echo '<pre>' . esc_html(print_r($existing,true)) . '</pre>';
            } else {
                echo '<p>No footer mapping.</p>';
            }
        }
