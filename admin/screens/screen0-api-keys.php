...
class Screen0_API_Keys {
    public function render() {
        echo '<div class="wrap">';
        echo '<div style="font-weight:bold; font-size:1.2em; margin-bottom:10px;">Screen 0 - API Keys</div>';
        echo '<h1>API Keys Manager</h1><form method="post">';
        wp_nonce_field('gurpovich_api_keys_action','gurpovich_api_keys_nonce');
        ...
        if (isset($_POST['gurp_inject'])) {
            if (!isset($_POST['gurpovich_api_keys_nonce']) || !wp_verify_nonce($_POST['gurpovich_api_keys_nonce'],'gurpovich_api_keys_action')) {
            ...
            // Save mapping meta
            update_post_meta($post_id,'gurp_api_keys_map',$map);
            echo '<div class="updated"><p>API keys mapping saved.</p></div>';
        ...
        // Show mapping
        echo '<h2>Current API Keys Mapping</h2>';
        if (!empty($_POST['gurp_post_id'])) {
            $existing = get_post_meta(intval($_POST['gurp_post_id']),'gurp_api_keys_map',true);
            if (is_array($existing)) {
                echo '<pre>' . esc_html(print_r($existing,true)) . '</pre>';
            } else {
                echo '<p>No API keys mapping.</p>';
            }
        }
